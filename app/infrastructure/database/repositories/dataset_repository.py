...
class DatasetRepository(IDatasetRepository):
    ...
    async def create_dataset(self, dataset: schemas.DatasetCreate, user_id: UUID) -> EntityDataset:
    ...
    async def get_dataset_by_id(self, dataset_id: UUID, user_id: Optional[UUID] = None) -> Optional[EntityDataset]:
    ...
    async def get_datasets(self, user_id: UUID, skip: int = 0, limit: int = 100, name_contains: Optional[str] = None) -> list[EntityDataset]:
    ...
    async def delete_dataset_by_id(self, dataset_id: UUID) -> None:
        ...
        if db_dataset:
            await self.db.delete(db_dataset)
            await self.db.commit()

...
from app.common.security.dependencies import require_roles, get_current_user
...
def get_datasets(skip: int = 0,limit: int = 100,name_contains: Optional[str] = None, current_user: UserEntity = Depends(get_current_user),db: Session = Depends(get_db), storage = Depends(get_storage)):
    service = DatasetService(DatasetRepository(db), storage)
    return service.get_datasets(current_user=current_user, skip=skip, limit=limit, name_contains=name_contains)


@router.get("/{dataset_id}", response_model=DatasetRead)
def get_dataset(dataset_id: UUID, current_user: UserEntity = Depends(get_current_user), db: Session = Depends(get_db), storage = Depends(get_storage)):
    service = DatasetService(DatasetRepository(db), storage)
...
def delete_dataset(dataset_id: UUID, current_user: UserEntity = Depends(get_current_user), db: Session = Depends(get_db), storage=Depends(get_storage), _: User = Depends(require_roles(["admin"]))):
    service = DatasetService(DatasetRepository(db), storage)
    try:

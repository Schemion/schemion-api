...
class DatasetService:
    ...
    async def create_dataset(self, dataset: DatasetCreate, file_data: bytes, filename: str, content_type: str, user_id: UUID) -> DatasetRead:
        validate_dataset_archive(file_data, filename)

        filepath = f"{user_id}/{filename}"

        dataset.minio_path = await self.storage.upload_file(file_data, filepath, content_type, settings.MINIO_DATASETS_BUCKET)
    ...
    async def get_dataset_by_id(self, dataset_id: UUID, user_id: UUID) -> Optional[
        ...
        await self.cache_repo.set(cache_key,  DatasetRead.model_validate(dataset).model_dump(), expire=CacheTTL.DATASETS.value)

        return dataset

    async def get_datasets(self, user_id: UUID, skip: int = 0, limit: int = 100, name_contains: Optional[str] = None) -> list[DatasetRead]:
        ...
        datasets = await self.dataset_repo.get_datasets(user_id=user_id, skip=skip, limit=limit, name_contains=name_contains)

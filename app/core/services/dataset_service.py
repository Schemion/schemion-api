...
class DatasetService:
    ...
    async def create_dataset(self, dataset: DatasetCreate, file_data: bytes, filename: str, content_type: str, current_user: entities.User) -> entities.Dataset:
        ...
        created_dataset = await self.dataset_repo.create_dataset(dataset, current_user.id)
    ...
    async def get_dataset_by_id(self, dataset_id: UUID, current_user: entities.User) -> Optional[entities.Dataset]:
        ...
        dataset = await self._ensure_dataset_exists(dataset_id, current_user.id)
    ...
    async def get_datasets(
        ...
        datasets = await self.dataset_repo.get_datasets(user_id=current_user.id, skip=skip, limit=limit, name_contains=name_contains)
    ...
    async def delete_dataset_by_id(self, dataset_id: UUID, current_user: entities.User) -> None:
        dataset = await self._ensure_dataset_exists(dataset_id, current_user.id)

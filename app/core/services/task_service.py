...
class TaskService:
    ...
    async def create_inference_task(self, task: TaskCreate, file_data: bytes, filename: str, content_type: str, current_user: entities.User) -> entities.Task:
    ...
    async def create_training_task(self, task: TaskCreate) -> entities.Task:
        await self._ensure_model_exists(task.model_id)
    ...
    async def get_task_by_id(self, task_id: UUID, current_user: entities.User) -> entities.Task:
    ...
    async def get_tasks(self, current_user: entities.User, skip: int = 0, limit: int = 100) -> list[entities.Task]:
        ...
        tasks = await self.task_repo.get_tasks(skip, limit, user_id=current_user.id)
    ...
    async def get_tasks_by_user_id(self, user_id: UUID) -> list[entities.Task]:
        return await self.task_repo.get_tasks_by_user_id(user_id)

    async def delete_task_by_id(self, task_id: UUID, current_user:entities.User) -> None:
        task = await self.task_repo.get_task_by_id(task_id)
        if not task:
            raise HTTPException(404, f"Task {task_id} does not exist")
